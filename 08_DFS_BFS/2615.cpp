...
const int SIZE = 20; //바둑판 크기
const int EMPTY = 0; //비어있는 곳
...
bool checkWin(matrix &board, int x, int y) {
    ...
    for (int idx = 0; idx < 4; idx++) {
        ...
        int prev_x = x - dx[idx], prev_y = y - dy[idx]; //이전 칸 좌표
        int next_x = x + dx[idx], next_y = y + dy[idx]; //다음 칸 좌표
...
int main() {
    // 입력
    matrix board(SIZE, vector<int>(SIZE, 0)); // 바둑판 0으로 초기화
    for (int i = 1; i < SIZE; i++)
        for (int j = 1; j < SIZE; j++)
            cin >> board[i][j]; //바둑판 상태 입력하기
